...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const params = { Bucket: process.env.AWS_S3_BUCKET!, Fields: { key }, Conditions: [['content-length-range', 1, 50 * 1024 * 1024]] };
  try {
    const presign = await s3.createPresignedPost({ Bucket: process.env.AWS_S3_BUCKET!, Key: key, Expires: 60 });
